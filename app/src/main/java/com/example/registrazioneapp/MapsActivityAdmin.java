...
public class MapsActivityAdmin extends FragmentActivity implements OnMapReadyCallback, LocationListener,GoogleMap.OnMarkerClickListener {
        ...
        databaseReference.addListenerForSingleValueEvent(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot dataSnapshot) {
                for(DataSnapshot s : dataSnapshot.getChildren() ){
                    ...
                    if(users.getMotivo().equals("ESCREMENTI")){
                    ...
                    if(users.getMotivo().equals("BUCA")){
                        LatLng location = new LatLng(users.getLat(),users.getLon());
                        mMap.addMarker(new MarkerOptions().position(location).title(users.getMotivo()))
                                .setIcon(BitmapDescriptorFactory
                                        .defaultMarker(BitmapDescriptorFactory.HUE_BLUE));
                    }
                    if(users.getMotivo().equals("GUASTO")){
                        LatLng location = new LatLng(users.getLat(),users.getLon());
                        mMap.addMarker(new MarkerOptions().position(location).title(users.getMotivo()))
                                .setIcon(BitmapDescriptorFactory
                                        .defaultMarker(BitmapDescriptorFactory.HUE_GREEN));
                    }
                    if(users.getMotivo().equals("RANDAGIO")){
                        LatLng location = new LatLng(users.getLat(),users.getLon());
                        mMap.addMarker(new MarkerOptions().position(location).title(users.getMotivo()))
                                .setIcon(BitmapDescriptorFactory
